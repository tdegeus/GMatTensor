...
class Test_tensor(unittest.TestCase):
    ...
    def test_I4d(self):
        ...
        tr = np.trace(A, axis1=-2, axis2=-1) / 3
        A_T = np.einsum("...ij -> ...ji", A)
        res = 0.5 * (A + A_T)
        res[..., 0, 0] -= tr
        res[..., 1, 1] -= tr
    ...
    def test_hydrostatic(self):
        ...
        res = np.trace(A, axis1=-2, axis2=-1) / 3
    ...
    def test_det(self):
        ...
        self.assertTrue(np.allclose(res, GMat.Det(A)))
        self.assertTrue(np.allclose(res, ret))
        ...
        for i in range(shape[0]):
            for j in range(shape[1]):
                for k in range(shape[2]):
                    res[i, j, k, :, :] = np.linalg.inv(B[i, j, k, :, :])

        ret = np.empty_like(res)
        GMat.inv(B, ret)

        self.assertTrue(np.allclose(res, GMat.Inv(B)))
        self.assertTrue(np.allclose(res, ret))
    ...
    def test_logs(self):
        ...
        for i in range(shape[0]):
            for j in range(shape[1]):
                for k in range(shape[2]):
                    val, vec = np.linalg.eig(B[i, j, k, :, :])
                    val = np.log(val)
                    for m in range(nd):
                        res[i, j, k, :, :] += val[m] * np.outer(vec[:, m], vec[:, m])
