...
namespace GMS = GMatTensor::Cartesian2dSym;
...
TEST_CASE("GMatTensor::Cartesian2dSym", "Cartesian2dSym.h")
{
    SECTION("Equivalent_deviatoric - Tensor2")
    {
        xt::xtensor<double, 2> A = xt::zeros<double>({2, 2});
        A(0, 1) = 1.0;
        A(1, 0) = 200.0; // ignores by symmetric implementation
        REQUIRE(GMS::Equivalent_deviatoric(A)() == Approx(std::sqrt(2.0)));
    }
    ...
    SECTION("Equivalent_deviatoric - List")
    {
        ...
        REQUIRE(xt::allclose(GMS::Equivalent_deviatoric(M), R));
    }
...
TEST_CASE("GMatTensor::Cartesian2dSym::pointer", "Cartesian2dSym.h")
{
    SECTION("deviatoric_ddot_deviatoric")
    {
        xt::xtensor<double, 2> A = xt::zeros<double>({2, 2});
        A(0, 1) = 1.0;
        A(1, 0) = 200.0; // ignores by symmetric implementation
        REQUIRE(GMS::pointer::deviatoric_ddot_deviatoric(A.data()) == Approx(2.0));
    }
    ...
    SECTION("A2_ddot_B2")
    {
        ...
        A(0, 1) = 1.0;
        A(1, 0) = 200.0; // ignores by symmetric implementation
        REQUIRE(GMS::pointer::A2_ddot_B2(A.data(), A.data()) == Approx(2.0));
    }
}
